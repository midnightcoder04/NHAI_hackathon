...
- [ ] T032 [P] [US2] **Frame-processor setup + FaceDetector.** First wire the VisionCamera v5 frame-processor runtime: install `react-native-worklets` + `react-native-vision-camera-worklets`, add `react-native-worklets/plugin` to `mobile/babel.config.js` (last in the plugins list), rebuild native, and land a trivial frame-processor smoke test (logs frame dims) before any ML. Then implement `FaceDetector` Vision Camera Frame Processor in `mobile/src/ml/FaceDetector.ts` using `react-native-fast-tflite` to run **BlazeFace f16** (`mobile/assets/models/blaze_face_short_range_float16.tflite`, `MODEL_FACE_DETECTOR`) on each camera frame — the loaded TFLite model must be boxed via `NitroModules.box()` so the worklet runtime can access it (fast-tflite's v5 frame-processor integration is community/undocumented; see plan.md Complexity Tracking); expose `detectFace(frame): { boundingBox, landmarks (6 pts), qualityScore } | null`. ⚠️ **Partially done** — JS-side `FaceDetector` implemented + unit-tested (`mobile/src/ml/FaceDetector.ts`: `computeQualityScore`/`selectBestDetection`/`parseDetection`/`detectFace`; `__tests__/unit/ml/FaceDetector.test.ts`, 8 tests). **Remaining (device build only):** `react-native-worklets` + `react-native-vision-camera-worklets` install, `react-native-worklets/plugin` in babel.config.js, `NitroModules.box()` of the BlazeFace model, native rebuild, and the frame-processor smoke test — these need a native build/emulator and could not be verified in CI here (adding the babel plugin before the package is installed breaks the jest run).
- [x] T033 [P] [US2] Implement `LivenessDetector` in `mobile/src/ml/LivenessDetector.ts` with two layers: (1) **active** — blink/head-turn via eye-aspect-ratio tracked across `LIVENESS_BLINK_FRAMES` consecutive frames using the MiniFASNet/landmarks f16 model (`face_landmarks_detector_float16.tflite`, `MODEL_LIVENESS_LANDMARKS`); (2) **passive** — run the Antispoof INT8 texture classifier (`antispoof_128x128_int8.tflite`, `MODEL_LIVENESS_ANTISPOOF`) on the 128×128 face ROI to detect printed-photo/screen-replay spoofs; expose `check(frames: Frame[]): 'live' | 'spoof' | 'inconclusive'`. Runs in the same VisionCamera v5 frame-processor/worklet context wired in T032 (shares `react-native-worklets` + boxed-model setup); the active layer accumulates eye-aspect-ratio across the live frame stream — this is the multi-frame requirement that drove choosing frame processors over still-capture. ✅ Done (TDD: T089 red→green) — pure dual-layer fusion: `eyeAspectRatio` + `detectBlink` (active) + `fuseLiveness`/`check` over the antispoof gate (passive). Model runners injected as deps (`runLandmarks`/`runAntispoof`) so the boxed-TFLite calls are the only native piece; that native invocation rides on the T032 frame-processor wiring (pending device build). New named constants `LIVENESS_EAR_CLOSED_THRESHOLD`, `LIVENESS_ANTISPOOF_REAL_THRESHOLD`.
...
- [ ] T046 [P] [US3] Implement `AuthService` in `mobile/src/services/AuthService.ts` using `@aws-sdk/credential-providers` `fromCognitoIdentityPool({ identityPoolId: AWS_COGNITO_IDENTITY_POOL_ID, clientConfig: { region } })` (guest / unauthenticated identity — no app login) to obtain temporary IAM credentials; expose `getCredentials(): Promise<AwsCredentialIdentity>` with in-memory caching and expiry refresh
