...
- [ ] T073 [P] Implement AES-256 field-level encryption at rest over **all PII** (FR-022) via a device-bound key held in `expo-secure-store`: create `mobile/src/services/CryptoService.ts` (encrypt/decrypt using `expo-crypto`), then encrypt-before-write / decrypt-after-read for `full_name`, `employee_id`, and the `embedding` BLOB in `PersonnelRepository.ts` and `FaceImageRepository.ts`, and encrypt the saved face image files in `ImageStorageService.ts`. (SQLCipher full-DB encryption noted as future upgrade — `expo-sqlite` does not bundle it.) ⚠️ **PARTIAL (2026-05-31, biometric-focused per stakeholder):** `CryptoService.ts` (AES-256-GCM via `@noble/ciphers`, device key in `expo-secure-store`, +5 unit tests) now encrypts the **face embedding BLOB** (FaceImageRepository) and **`full_name`** (PersonnelRepository) at rest — decrypt-on-read with legacy-plaintext tolerance; `employee_id` left plaintext (UNIQUE / RDS-upsert key) and the transient sync_outbox payload left plaintext (SigV4 TLS). **DEFERRED: encrypting the saved face-image FILES** — it requires rewiring the photo thumbnail + S3 upload to a decrypt-to-cache path (visible-regression risk; needs on-device verification).
- [x] T074 Handle device storage full edge case in `ImageStorageService.ts`: call `FileSystem.getFreeDiskStorageAsync()` before saving; if available bytes < 2× image size, throw a typed `StorageFullError` and surface a React Native Paper Snackbar in `PersonnelDetailScreen.tsx`
- [x] T075 Add camera permission handling in `PersonnelDetailScreen.tsx`: check `Camera.getCameraPermissionStatus()` on mount; if not granted, request permission; if permanently denied, show a "Go to Settings" button
- [x] T076 Handle RDS schema-rejected records in `SyncService.ts`: on `400 VALIDATION_ERROR` response, log the full `details` array to console, mark each affected `sync_outbox` entry as `failed` with the field-level message, and do not block healthy records in the same batch from being retried
- [x] T077 Guard personnel delete during active sync in `PersonnelRepository.delete`: if the personnel record's `sync_status` is not `synced` or a `sync_outbox` entry for that record is `in_progress`, enqueue the delete as a tombstone entry rather than immediately removing the row; add a `tombstoned` column to the `personnel` SQLite table and filter it from `PersonnelListScreen`
- [x] T078 [P] Add React Native Paper theme configuration in `mobile/src/components/theme.ts`: define primary, secondary, and error colours; wrap `App.tsx` root with `PaperProvider theme={theme}`; apply `200 ms` activity indicator delay rule across all async operations (show loading state only after 200 ms to avoid flicker)
- [x] T079 [P] Add `infra/docker-compose.yml` with a LocalStack service (image `localstack/localstack`) exposing SQS, S3 on `localhost:4566`, and document local Lambda integration test setup in `quickstart.md`
...
- [ ] T100 [P] Performance benchmark harness `mobile/__tests__/perf/inference.bench.ts`: assert per-stage budget (BlazeFace ~20 ms, MobileFaceNet ~60 ms, MiniFASNet ~100 ms, Antispoof ~30 ms; ≈ 210 ms total) and end-to-end verification < 1 s (SC-002); also assert SC-003 ≥ 95 % spoof rejection and SC-004 ≥ 90 % match accuracy against a labelled fixture set; record results in `plan.md` Profiling Results. ⚠️ **PARTIAL (2026-06-05): SC-004 measured offline against an external dataset.** `scripts/validate_recognition_lfw.py` simulates the pipeline (ai_edge_litert) over `sklearn.datasets.fetch_lfw_pairs('test')` — 1000 pairs through real **BlazeFace f16 → MobileFaceNet INT8** (per-pair BlazeFace box/keypoints/conf trace printed → fed into the embedder → cosine prediction vs ground truth). **Result: as-shipped pipeline AUC 0.794 / 71.3 % @ `FACE_MATCH_THRESHOLD`=0.65 — BELOW SC-004.** Adding 5-point ArcFace alignment lifts to **AUC 0.963 / 92.4 % @ ~0.42**, clearing SC-004. **✅ (2026-06-05) ALIGNMENT NOW IMPLEMENTED ON-DEVICE.** Closed-form 2D-similarity solve (`solveSimilarityTransform` + `ARCFACE_TEMPLATE_112` in `mobile/src/ml/preprocessing.ts`, complex least-squares = reflection-free Umeyama, no SVD; +3 unit tests) inlined into the `frameProcessor.ts` IDENTITY block (per-pixel inverse-warp + nearest sample, degenerate-keypoint fallback). The **exact shipped math** is mirrored + measured in `validate_recognition_lfw.py --align --align-method similarity-nearest`: **AUC 0.950 / best-acc 91.2 % @ 0.43 — clears SC-004** (≈0.013 AUC below the SVD/bilinear reference = the nearest-neighbour worklet trade-off). **✅ (2026-06-06) CROP TIGHTENED + THRESHOLD RECALIBRATED.** A crop-tightness sweep (`scripts/sweep_crop_margin.py`) refuted the "detector crop too tight (skips hair)" hypothesis — looser crops hurt monotonically (hair/background = shared identity-noise that inflates impostor similarity); a *slight* tighten peaks at **1.15× about the template centroid** (`ARCFACE_CROP_TIGHTEN=1.15`, derives `ARCFACE_TEMPLATE_112`; worklet `dxs/dys` updated): AUC **0.950→0.961**. A model comparison (`compare_mfn_variants.py` + `make_fp16.py`) found float32 & float16 (192-d) reach AUC 0.968 but need a 128→192-d embedding-schema migration + full re-enrollment → **int8 retained** (tightened crop = the free win). **`FACE_MATCH_THRESHOLD` recalibrated 0.65 → 0.45** from the FAR/FRR curve (`scripts/threshold_report.py`): FAR ≈1.4 % / FRR ≈16.8 % / acc 90.9 % (**SC-004 PASS at the gate**), prec 98.3 %, recall 47 %→83 %; `FACE_LOW_CONFIDENCE_THRESHOLD` 0.5 → 0.40 in lockstep. Caveat: LFW is hard (look-alike impostors) → field FAR lower; re-confirm on real captures. See `plan.md` → Profiling Results. **➕ (2026-06-05) SC-002 latency + SC-003 anti-spoof harness added.** `scripts/benchmark_models.py` times all four models on dev CPU (BlazeFace 0.6 / MobileFaceNet 11.4 / FaceMesh 2.4 / Antispoof 2.2 ms mean; Σ ≈ 17 ms — SC-002 < 1 s passes with wide margin, all under the ~210 ms budget; caveat: CPU-only, not the Snapdragon 665 target → on-device latency is T102; INT8 antispoof is *slower* than f16/f32 on CPU w/o NNAPI). `scripts/validate_antispoof.py` simulates the PASSIVE path (BlazeFace → 1.5× crop → Antispoof → softmax `p[real]` @ gate 0.5) with ISO 30107-3 APCER/BPCER/ACER and a real-PAD certification path (`--live-dir`/`--spoof-dir`). **SC-003 result: NOT certifiable offline — UNVALIDATED.** PROXY (LFW bona-fide + synthetic recapture) gives ROC-AUC ≈0.51 / 54 % spoof-rej / 49 % BPCER — confounded (LFW are archival photos, OOD for a live-vs-recapture model; synthetic artefacts ≠ real attack distribution), so this is a red-flag that the gate has no real-data evidence, *not* a proof of failure. **🔎 (2026-06-05) ROOT CAUSE — antispoof preprocessing bug (`scripts/diagnose_antispoof.py`).** A model×colour×normalisation sweep proves (i) **model variant is irrelevant** (int8≈f16≈f32 within ±0.01 AUC — switching to f16/f32 does *not* help), and (ii) **the app preprocesses the antispoof input wrong**: it feeds **RGB + ImageNet mean/std**, but the model wants **BGR + plain /255 ([0,1])**. Correcting it lifts genuine-face acceptance **46 %→98 %** (BPCER 49 %→0 %), separation AUC **0.47→0.74**, and de-saturates the logits — the shipped config is near the *worst* of 24 combos. Necessary fix but **not sufficient** for SC-003 (synthetic attacks still ~0.95). **⚠️ (2026-06-05) BGR fix shipped — LATER SUPERSEDED by real data (below).** **✅ (2026-06-06) SC-003 CERTIFIED on real PAD data + antispoof re-corrected.** Real attack set arrived (`LiveSpoofDataset/`: 3,062 live + 3,107 spoof, pre-cropped 112²). Held-out calibration (`scripts/compare_antispoof_variants.py`, `investigate_antispoof_crop.py`) overturns the synthetic conclusion: the shipped **BGR + p[1]** config was *inverted/broken* on real data (AUC **0.26** — accepts ~87% spoofs, rejects ~40% live). Correct config = **RGB + /255 + realProb=p[0]** (class 0 = real), AUC **0.81**. **Model variant still irrelevant** (int8≈f16≈f32, AUC spread 0.0003 — confirmed on real data). **Crop loosened 1.5→2.0×** (`ANTISPOOF_CROP_SCALE`; model wants the face small with border, AUC 0.81→0.84). APPLIED: `preprocessAntispoof`(RGB), `antispoofRealProbability`(p[0]), worklet (RGB+p[0]+crop), `test_liveness.py`, `validate_antispoof.py`, `preprocessing.test.ts` (jest green, tsc clean). **SC-003 VERDICT: CERTIFIED but BELOW TARGET ⚠️** — even corrected, the model caps at ~0.81–0.84 AUC / **67–74% spoof-rejection**; reaching 95% needs a gate that rejects ~75% of genuine users → **model-capability gap, needs a stronger passive anti-spoof model** (not threshold tuning). **✅ (2026-06-05) ON-DEVICE ANTISPOOF RESIZE DE-ALIASED (instability fix).** Field report: on-device `realProb` flickered REAL↔SPOOF on a perfectly still face, while `test_liveness.py` stayed stable. Root cause = the resize FILTER, not the math (channel/norm/crop/scale/softmax-index all already matched the script). The worklet downsampled the antispoof crop to 128² with **nearest-neighbour**, whereas the canonical scripts use **bilinear** (`cv2.resize` / `PIL.BILINEAR`). NN decimation aliases the print/screen texture this model keys on into a *phase-sensitive* pattern that shifts with the detector's frame-to-frame box jitter → the verdict flapped across the 0.85 gate. FIX: replaced the NN crop→128 loop in `frameProcessor.ts` PASSIVE block with **area-averaging (box-filter)** (mean of each dest pixel's source footprint = correct anti-alias for a >2× downscale), mirrored by the new unit-tested `resizeRgbAreaAverage` in `preprocessing.ts` (+4 tests). Embedding/FaceMesh sampling left on nearest (the embedding NN trade-off is already measured/calibrated above; EAR is a landmark ratio, resize-robust). jest 249/249, tsc + eslint clean. Stabilises the verdict; does NOT raise the model ceiling. *(Note for later: this note records `ANTISPOOF_CROP_SCALE` "loosened 1.5→2.0" but `constants/index.ts` ships 1.5 — device & `test_liveness.py` agree at 1.5; reconcile when the stronger model lands.)* **Still pending for full T100:** swap in a stronger antispoof model to actually meet SC-003 (then re-certify on `LiveSpoofDataset/`); confirm `ANTISPOOF_CROP_SCALE` on real frames; the `inference.bench.ts` TS harness + per-stage budget asserts wired into CI (T103); and the on-device latency profile (T102).
- [x] T101 [P] Sync throughput benchmark: 500 verification records sync within 3 min over stable connectivity (SC-005); fail on > 20 % regression. ✅ `mobile/__tests__/perf/sync-throughput.bench.ts` drives the REAL outbox dispatch loop (`runDispatchCycle`) over a 500-record node:sqlite outbox with network/creds/FS stubbed; asserts (1) elapsed < SC-005 180 s spec gate, (2) elapsed < a conservative CI ceiling (>20 % regression guard), (3) all 500 acked in one cycle. Runs via `pnpm test:perf` (outside the default testMatch) and is wired into CI (T103).
- [ ] T102 Memory/CPU profiling pass on a mid-range device profile (Snapdragon 665 / 3 GB RAM): record peak memory, CPU, and per-stage timings into `plan.md` Profiling Results before demo (Constitution V). ⏳ **NEEDS PHYSICAL DEVICE — not automatable from CI.** Capture with `adb shell dumpsys meminfo com.anonymous.nhaifieldops` + Android Studio profiler while running the verification loop on a Snapdragon-665-class phone; the on-device per-stage latency (the device half of T100) is recorded here. Offline CPU latencies already in `scripts/benchmark_models.py` (Σ≈17 ms dev CPU).
- [x] T103 [P] CI pipeline `.github/workflows/ci.yml`: pnpm install → lint (zero violations) → typecheck → jest with coverage gate (≥ 80 % line / ≥ 70 % branch) → contract tests → perf benchmark step (fail on > 20 % regression). Enforces all Constitution Quality Gates. ✅ Three jobs: **mobile** (install → `lint` → `typecheck` → `test:cov` with the coverageThreshold gate in package.json → `test:contract` isolated → `test:perf`), **lambda** (typecheck → coverage), **terraform** (fmt-check non-blocking → init -backend=false → validate). Node 24 / pnpm 10.33, frozen lockfile, concurrency-cancel on ref.
- [x] T104 [P] Accessibility pass (WCAG 2.1 AA) across all four screens: `accessibilityLabel`/`accessibilityRole` on every interactive element, ≥ 4.5:1 text contrast in the Paper theme, ≥ 48 dp touch targets, logical screen-reader focus order, and outcome results conveyed by text+icon (not colour alone) — Constitution IV. ✅ **AUDITED + GAP FIXED (2026-06-05).** Theme contrast (white on `primary #1B7A43` ≈5.3:1, `error #B3261E` ≈6.4:1, `secondary #37607D` ≈6.3:1) all clear AA ≥4.5:1. Touch targets: Paper FAB/Button/Appbar.Action default ≥48 dp. Outcomes conveyed by **icon + text** (`VerificationStatusBanner` has `accessibilityRole` alert/progressbar/summary + label, never colour-only). Labels: header actions ("Verify personnel"/"Backup status") + Paper inputs/buttons carry text labels; **fixed the one gap** — the register FAB had no label → added `accessibilityLabel="Register personnel"` (`PersonnelListScreen.tsx`). Focus order follows source order (no manual reorder needed).
- [x] T105 [P] Maestro E2E flows in `mobile/.maestro/`: (a) US1 register → list → edit → delete; (b) US2 verify authorized / unauthorized / liveness-failed; run headless in CI (supersedes the manual-only golden path in T080). ✅ Authored `mobile/.maestro/us1_personnel_crud.yaml` (full CRUD, camera-free, deterministic) and `us2_verification.yaml`. **SCOPE NOTE on (b):** Maestro cannot inject camera frames, so the verify OUTCOMES (authorized/unauthorized/liveness-failed) can't be driven E2E — those are covered deterministically by `VerificationScreen.test.tsx` (authorized · Potential Spoof · timeout · capture-failure); the Maestro US2 flow asserts launch + camera-permission handling + entry into the continuous "position face" loop. (Headless-in-CI needs a Maestro+emulator runner; the YAML is the deliverable, wiring is an infra step alongside T102.)
