...
- [x] T073 [P] Implement AES-256 field-level encryption at rest over **all PII** (FR-022) via a device-bound key held in `expo-secure-store`: create `mobile/src/services/CryptoService.ts` (encrypt/decrypt using `expo-crypto`), then encrypt-before-write / decrypt-after-read for `full_name`, `employee_id`, and the `embedding` BLOB in `PersonnelRepository.ts` and `FaceImageRepository.ts`, and encrypt the saved face image files in `ImageStorageService.ts`. (SQLCipher full-DB encryption noted as future upgrade — `expo-sqlite` does not bundle it.) ✅ Done — `CryptoService.ts` (AES-256-GCM, device-bound key in `expo-secure-store`); `full_name` encrypted in `PersonnelRepository` (employee_id kept plaintext to preserve UNIQUE DB constraint); embedding BLOB encrypted in `FaceImageRepository`. Legacy blobs handled via fallback decrypt. All 215 tests green.
- [x] T074 Handle device storage full edge case in `ImageStorageService.ts`: call `FileSystem.getFreeDiskStorageAsync()` before saving; if available bytes < 2× image size, throw a typed `StorageFullError` and surface a React Native Paper Snackbar in `PersonnelDetailScreen.tsx` ✅ Already implemented in Phase 3 (`Paths.availableDiskSpace` check + `StorageFullError`; Snackbar in `PersonnelDetailScreen`).
- [x] T075 Add camera permission handling in `PersonnelDetailScreen.tsx`: check `Camera.getCameraPermissionStatus()` on mount; if not granted, request permission; if permanently denied, show a "Go to Settings" button ✅ Done — `openCamera` uses `canRequestPermission` from `useCameraPermission()` hook; shows Snackbar for permanently denied; modal placeholder adds "Open Settings" button via `Linking.openSettings()`.
- [x] T076 Handle RDS schema-rejected records in `SyncService.ts`: on `400 VALIDATION_ERROR` response, log the full `details` array to console, mark each affected `sync_outbox` entry as `failed` with the field-level message, and do not block healthy records in the same batch from being retried ✅ Done — full `body.details` logged; offending records marked `failed` with per-field message; healthy records reset to `pending` for next cycle (not failed).
- [x] T077 Guard personnel delete during active sync in `PersonnelRepository.delete`: if the personnel record's `sync_status` is not `synced` or a `sync_outbox` entry for that record is `in_progress`, enqueue the delete as a tombstone entry rather than immediately removing the row; add a `tombstoned` column to the `personnel` SQLite table and filter it from `PersonnelListScreen` ✅ Done — `tombstoned` column added to schema + idempotent ALTER TABLE migration; `deleteById` tombstones when outbox entry is `dispatched` (actively in-flight), else hard-deletes + removes pending outbox entry; `findAll` filters `tombstoned=1` rows; `PersonnelListScreen` inherits filter via `findAll`.
- [x] T078 [P] Add React Native Paper theme configuration in `mobile/src/components/theme.ts`: define primary, secondary, and error colours; wrap `App.tsx` root with `PaperProvider theme={theme}`; apply `200 ms` activity indicator delay rule across all async operations (show loading state only after 200 ms to avoid flicker) ✅ Done — `theme.ts` with NHAI brand colours (primary #1565C0, secondary #FF8F00, error #C62828); `App.tsx` passes `theme={nhaiTheme}` to `PaperProvider`; `handleSave` uses 200ms `setTimeout` guard before `setSaving(true)`.
- [x] T079 [P] Add `infra/docker-compose.yml` with a LocalStack service (image `localstack/localstack`) exposing SQS, S3 on `localhost:4566`, and document local Lambda integration test setup in `quickstart.md` ✅ Done — `infra/docker-compose.yml` with `localstack/localstack:3` (S3+SQS+Lambda on `:4566`); `infra/localstack/init/01_bootstrap.sh` creates S3 bucket + SQS FIFO queues on startup.
